...
app.use( async (ctx)  => {
    ...
    switch (method) {
        case 'allTickets':
          ...
          ctx.response.body = tickets.map( el => {return {id: el.id, name: el.name, description: el.description, status: el.status, created: el.created}});
          return;
        case 'ticketById':
          ctx.response.status = 200;
        ...
        case 'createTicket':
          let gId = uuid.v4();
          let checkExist = tickets.filter(el => el.name === ctx.body.name &&  el.description === ctx.body.description);
